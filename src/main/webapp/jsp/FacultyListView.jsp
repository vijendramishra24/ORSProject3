<%@page import="in.co.sunrays.proj3.ctl.FacultyListCtl"%>
...
<script type="text/javascript">
 ...
 %>
  <jsp:useBean id="bean" class="in.co.sunrays.proj3.dto.FacultyDTO"
	...
	<h1><font style=" color:grey;  font-style: italic; font-weight: bold;">Faculty List</font></h1>

        <form class="form-inline"  action="<%=ORSView.FACULTY_LIST_CTL%>" method="post" >
    ...
    <table class="table" >
				<tr align="center">
					...
					<%=HTMLUtility.getList("courseList", String.valueOf(bean.getCourseId()), l)%>
						...
						&emsp;
						<button class="btn btn-default " type="submit" name="operation" value="<%=FacultyListCtl.OP_SEARCH%>">
							<span class="glyphicon glyphicon-search"></span> <%= FacultyListCtl.OP_SEARCH%></button>
						
						&emsp; 
						<button class="btn btn-default " type="submit" name="operation" value="<%=FacultyListCtl.OP_RESET%>">
							<span class="glyphicon glyphicon-refresh "></span> <%= FacultyListCtl.OP_RESET%></button>
           ...
           <tr>
			...
			<%
				...
				%>
					...
					<button type="submit" class="btn btn-default" name="operation" value="<%=FacultyListCtl.OP_ADD%>">
					<span class="glyphicon glyphicon-plus"> </span> <%= FacultyListCtl.OP_ADD%></button>
					</td>
				<%
					}else{
				%>
	...
	<%
               ...
               <%
                ...
                CollegeDTO collegebean = college.findByPK(collegeId);
                collegebean.getName();
                %>
                <%
				...
				<tr>
					...
					<td align="center" style="border-color: black;"><a href="FacultyCtl?id=<%=bean.getId()%>"> 
			...
			<table class="table" >
				<tr >
					...
					<button type="submit" class="btn btn-default" name="operation" value="<%=FacultyListCtl.OP_PREVIOUS%>" <%=(pageNo <= 1) ? "disabled" : ""%>> 
					<span class="glyphicon glyphicon-menu-left"> </span> <%= FacultyListCtl.OP_PREVIOUS%></button>
					</td>
					...
					<td style="border-color: transparent;" align="center">
					<button type="submit" class="btn btn-default" name="operation" value="<%=FacultyListCtl.OP_DELETE%>"
						<%=(list.size() == 0) ? "disabled" : ""%>><span class="glyphicon glyphicon-trash"></span> <%= FacultyListCtl.OP_DELETE%></button>
					</td>
					...
					%>
					<td style="border-color: transparent;" align="center">
					<button type="submit" class="btn btn-default" name="operation" disabled="disabled" value="<%=FacultyListCtl.OP_NEXT%>">
					<%=FacultyListCtl.OP_NEXT %> <span class="glyphicon glyphicon-menu-right"></span> </button>
					<%
						} else {
					%>
					<td style="border-color: transparent;" align="center">
					<button type="submit" class="btn btn-default" name="operation"  value="<%=FacultyListCtl.OP_NEXT%>">
					<%=FacultyListCtl.OP_NEXT %> <span class="glyphicon glyphicon-menu-right"></span></button>
					<%
						}
					%>
